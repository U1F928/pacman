...
std::shared_ptr<GameEntity> Teleport::update(const GameState& gameState) const
{
    ...
    if (updatedTeleport->nearestTeleportPosition == std::make_pair(-1, -1))
    {
        ...
        log("nearest teleport pos: ");
        log(updatedTeleport->nearestTeleportPosition.first);
        log(" ");
        log(updatedTeleport->nearestTeleportPosition.second);
        log("\n");
        log("this teleport pos: ");
        log(updatedTeleport->position.first);
        log(" ");
        log(updatedTeleport->position.second);
        log("\n");
    }
